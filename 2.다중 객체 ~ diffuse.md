...
`hittable_list`도 `hittable`을 상속한다는 게 포인트. 그래서 `ray_color`는 `world.hit()` 하나만 부르면 됨.
...
```
안쪽 `if`에서 `auto root`로 또 선언하면 새 변수가 됨(이름만 같음). `}` 나가면 사라짐.
...
- `closest_so_far`는 `ray_tmax` 자리에 들어가서 `sphere::hit` 안에서 `t`랑 비교됨. 그러니 t 단위여야 함.
...
- `virtual`: `hittable*` 포인터로 불러도 실제 가리키는 객체(sphere)의 hit이 불리게 하는 스위치.
...
해법: 픽셀 하나에 광선 여러 개를 랜덤 위치로 쏘고 색을 평균. 경계 픽셀은
...
`rtweekend.h`가 `vec3.h`/`ray.h`를 include하고, 나머지 헤더(camera, sphere 등)는 `rtweekend.h`를 include하는 구조.
...
핵심: 광선이 표면 닿으면 랜덤 방향으로 새 광선을 만들어 튕김 → 그게 또 어딘가 닿아 또 튕김 → ...
→ 하늘(배경)에 닿으면 하늘색을 얻고, 튕길 때마다 색이 감쇠(×0.5)되며 되돌아옴.
→ 이걸 재귀로 구현. `ray_color`가 자기를 부름.
...
자기가 방금 출발한 표면을 t≈0에서 다시 맞았다고 착각함 → 표면을 못 벗어나고 그 자리서 계속 튕겨 → 검은 얼룩 범벅.
`t_min = 0.001`로 "출발점 바로 근처(t < 0.001) 교차 무시" → 자기 표면 오인 제거.

> CUDA 북마크: 이 재귀 `ray_color`가 GPU에선 반복문 + 깊이 제한으로 바뀜(깊은 재귀가 GPU에 부담).
...
→ 미리 sqrt를 취해 밝혀서 보내면 모니터의 제곱과 상쇄됨.
